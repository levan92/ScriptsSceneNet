...
def main():
    csv_path = "/homes/el216/Workspace/SUNCGtoolbox/metadata/ModelCategoryMapping.csv"
    ...
    for line in csv:
        ...
        if wnid.startswith('n'):
            wnid = wnid.strip('n')
            if wnid not in wnid_in_suncg:
                wnid_in_suncg.append(wnid)
                names.append(desc)

    wnid_in_label = WNID_TO_NYU_CLASS.keys()
    i = 0
    for wnid in wnid_in_suncg:
        if wnid not in wnid_in_label:
            print "'{}':{}, ".format(wnid,names[i])
        i += 1
